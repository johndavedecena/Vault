...
*/
	...
	public function index($assetClass,$sortby=null,$order=null){
		...
		if(Session::has('username') && (Session::get('user_type')=="Root" || Session::get('user_type')=="Admin" || Session::get("user_type")=="User")){
			...
			$view = View::make("Assets.Network.network_assets");
			$view->nav = "assets";
			$view->tab = $assetClass;
				
			$checkClass = AssetClassification::where("url_key","=",$assetClass)->where("type","=","Network")->first();
			...
			if(empty($sortby) || empty($order)){
				...
				if(!in_array($sortby, array("asset_tag","serial_number","model_id","warranty_start","warranty_end")) || !in_array($order,array("asc","desc"))){
					return Redirect::to("assets/network/view/".$assetClass);
				}
	...
	public function search($assetClass){
		...
		if(Session::has('username') && (Session::get('user_type')=="Root" || Session::get('user_type')=="Admin" || Session::get("user_type")=="User")){
			...
			if(!AssetClassification::where("url_key","=",$assetClass)->where("type","=","Network")->first()){
				return Redirect::to("assets/network/view/accesspoints");
			}
			...
			if(!empty($keyword)){
				...
				return Redirect::to('assets/network/search/'.$assetClass.'/'.urlencode($keyword))->withInput();
			}
	...
	public function addAsset($assetClass){
		...
		if(Session::has('username') && (Session::get('user_type')=="Root" || Session::get('user_type')=="Admin")){
			...
			$view = View::make("Assets.Network.add_network_asset");
		...
		if(Session::has('username') && (Session::get('user_type')=="Root" || Session::get('user_type')=="Admin")){
			...
			$validator = Validator::make(
					...
					array(
							...
							"status"=>"required|in:Available,For Repair,Installed,Lost,Retired"
					)
			);
	
			if($validator->fails()){
				Input::flash();
				return Redirect::to('assets/network/add/'.$input["asset_key"])->with('message', $validator->messages()->first());
			}
				...
				Input::flash();
				return Redirect::to('assets/network/add/'.$input["asset_key"])->with('message', "Cannot assign asset to employees no longer working in the company.");
			}
				...
				return Redirect::to('assets/network/add/'.$input["asset_key"])->with('success',"You have successfully added a new network asset.");
	...
	public function updateAsset($assetClass,$id){
		...
		if(Session::has('username') && (Session::get('user_type')=="Root" || Session::get('user_type')=="Admin")){
			...
			$view = View::make("Assets.Network.update_network_asset");
			$view->nav = "assets";
			$view->tab = $assetClass;
	...
	public function logAsLost($id){
		...
		if(Session::has('username') && (Session::get('user_type')=="Root" || Session::get('user_type')=="Admin")){
			...
			$view = View::make("Assets.Network.network_asset_lost");
			...
			if($asset->status=="Lost"){
				return Redirect::to("assets/network/update/".$asset->classification->url_key."/".$asset->id);
			}
	...
	public function lost(){
		...
		if($asset->status=="Lost"){
			...
			if($validator->fails()){
				return Redirect::to("assets/network/lostasset/".Input::get("id"))->withInput()->with("message",$validator->messages()->first());
			}
	
			else if($input["employee_number"]!=null && !empty($asset->employee->last_name) && $asset->employee_number==$input["employee_number"]){
				return Redirect::to("assets/network/lostasset/".Input::get("id"))->withInput()->with("message","Cannot reassign an asset to the same employee.");
			}
				...
				if(!empty($asset->employee->last_name)){
					...
					if($input["employee_number"]!=null){
						...
						$desc = "Network Asset <strong>".$asset->asset_tag."</strong>, SN: <strong>".$asset->serial_number."</strong> lost by <strong>".$asset->employee->first_name." ".$asset->employee->last_name."</strong> and assigned to employee <strong>".$employee->first_name." ".$employee->last_name."</strong>.";
						...
						$desc = "Network Asset <strong>".$asset->asset_tag."</strong>, SN: <strong>".$asset->serial_number."</strong> lost by <strong>".$asset->employee->first_name." ".$asset->employee->last_name."</strong>.";
					...
					if($input["employee_number"]!=null){
						...
						//Get the employee
						$employee = Employee::where("employee_number","=",$input["employee_number"])->first();
						$desc = "Network Asset <strong>".$asset->asset_tag."</strong>, SN: <strong>".$asset->serial_number."</strong> lost and assigned to <strong>".$employee->first_name." ".$employee->last_name."</strong>.";
						...
						$asset->status = "Lost";
						$asset->save();
						$desc = "Network Asset <strong>".$asset->asset_tag."</strong>, SN: <strong>".$asset->serial_number."</strong> lost.";
					}
				}
				...
				return Redirect::to("assets/network/update/".$asset->classification->url_key."/".Input::get("id"))->with("success","You have successfully logged the transaction.");
	...
	public function transferAsset($id){
		...
		if(Session::has('username') && (Session::get('user_type')=="Root" || Session::get('user_type')=="Admin")){
			...
			$view = View::make("Assets.Network.network_transfer_asset");
		...
		if(Session::has('username') && (Session::get('user_type')=="Root" || Session::get('user_type')=="Admin")){
			...
			$validator = Validator::make(
					...
					array(
							...
							"asset status"=>"required|in:Available,For Repair,Installed,Retired"
					)
			);
				
			if($validator->fails()){
				return Redirect::to("assets/network/transferasset/".Input::get("id"))->withInput()->with("message",$validator->messages()->first());
			}
				
			else if($input["employee_number"]!=null && !empty($asset->employee->last_name) && $asset->employee_number==$input["employee_number"]){
				return Redirect::to("assets/network/transferasset/".Input::get("id"))->withInput()->with("message","Cannot transfer an asset to the same employee.");
			}
				...
				Input::flash();
				return Redirect::to("assets/network/transferasset/".Input::get("id"))->with('message', "Cannot assign asset to employees no longer working in the company.");
			}
				...
				if(!empty($asset->employee->last_name)){
					...
					//Get the employee
					$employee = Employee::where("employee_number","=",$input["employee_number"])->first();
					$desc = "Network Asset <strong>".$asset->asset_tag."</strong>, SN: <strong>".$asset->serial_number."</strong> transferred from <strong>".$asset->employee->first_name." ".$asset->employee->last_name."</strong> and assigned to employee <strong>".$employee->first_name." ".$employee->last_name."</strong>, with status <strong>".$input["asset_status"]."</strong>.";
					...
					//Get the employee
					$employee = Employee::where("employee_number","=",$input["employee_number"])->first();
					$desc = "Network Asset <strong>".$asset->asset_tag."</strong>, SN: <strong>".$asset->serial_number."</strong> transferred to <strong>".$employee->first_name." ".$employee->last_name."</strong>, with status <strong>".$input["asset_status"]."</strong>.";
				...
				return Redirect::to("assets/network/transferasset/".Input::get("id"))->with("success","You have successfully transferred the asset.");
	...
	public function import(){
		...
		if(Session::has('username') && (Session::get('user_type')=="Root" || Session::get("user_type")=="Admin")){
			$view = View::make("Assets.Network.import_network");
			$view->nav = "assets";
			$view->tab = "import";
	...
	public function postImport(){
		...
		if(Session::has('username') && (Session::get('user_type')=="Root" || Session::get('user_type')=="Admin")){
			...
			if($validator->fails()){
				return Redirect::to('assets/network/import')->withInput()->with('message',"Please select an asset type.");
			}
			
			else if(!AssetClassification::where("id","=",Input::get("asset_class_id"))->where("type","=","Network")->first()){
				return Redirect::to('assets/network/import')->withInput()->with('message',"Please select a network asset type.");
			}
	
			else if(!Input::hasFile('file')){
				return Redirect::to('assets/network/import')->withInput()->with('message',"Please select a valid excel file.");
			}
	...
	private function importResult($fileHasError, $hasCorrectRows, $rowsWithErrors=null, $errorDetails=null){
		...
		if(Session::has('username') && (Session::get('user_type')=="Root" || Session::get('user_type')=="Admin")){
			...
			$view = View::make("Assets.Network.import_network_result");
	...
	public function redirector($assetClassId,$intent,$id=null){
		...
		if(Session::has('username') && (Session::get('user_type')=="Root" || Session::get('user_type')=="Admin")){
			...
			$assetClassification = AssetClassification::where("id","=",$assetClassId)->where("type","=","Network")->first();
			...
			if($intent=="add"){
				return Redirect::to("assets/network/add/".$assetClassification->url_key);
			}
				
			return Redirect::to("assets/network/update/".$assetClassification->url_key."/".$id);
		}
